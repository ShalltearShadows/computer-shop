<template>
    <div>
        ...
        <el-dialog title="查看物流进度" :visible.sync="progressDialogVisible" width="50%">
            <!-- 时间线 -->
            <el-timeline>
                <el-timeline-item v-for="(activity, index) in progressInfo" :key="index" :timestamp="activity.time">{{activity.content}}</el-timeline-item>
            </el-timeline>
        </el-dialog>
    </div>
</template>
    ...
    export default {
        data() {
            return {
                ...
                // 物流进度
                progressInfo: [
                    {content: '活动按期开始', time: '2018-04-15' },
                    {content: '通过审核', time: '2018-04-13' },
                    {content: '创建成功', time: '2018-04-11' }
                ]
            }
        },
        created() {
